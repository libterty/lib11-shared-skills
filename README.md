...
這個 repo 可以獨立使用，也可以當作 git submodule 掛在其他專案下（例如 `going-cloud` 就是以 submodule 形式引用這裡）。
...
Claude Code 的 Skill 工具是從 `.claude/skills/<name>/SKILL.md` 探索 skill 的，不是這裡的 `shared-skills/`。如果這個 repo 是被當成 submodule 掛在某個專案下，該專案通常會提供一支安裝腳本（例如 `going-cloud` 的 `scripts/install-claude-skills.sh`），把 `shared-skills/<name>/` symlink 到 `.claude/skills/<name>/`，讓 Claude Code 原生列出並主動提議使用。
