...
1) 把 Fireflies（或任何來源）產出的逐字稿或 AI 摘要貼給 notes-to-action-digest：

   「請照 notes-to-action-digest/SKILL.md 的定義，幫我拆解這份會議摘要：
   [貼上完整逐字稿/AI 摘要文字，例如一份公司經營訪談摘要]」

   → 得到決策事項/待辦/待釐清問題/純資訊/pattern 五個區塊，例如：

   決策事項：（無——本次輸入為單一敘事摘要，文字中沒有明確的多方共識陳述）

   待辦事項：
   1. 內容：繼續加大在AI相關快閃記憶體技術及企業級SSD的研發投入，保持技術領先
      負責人：潘建成／截止日期：未說明／Source ID：06:25
   2. 內容：持續專注快閃記憶體控制器核心業務，避免無謂業務擴張，維持公司專業度及市場優勢
      負責人：潘建成／截止日期：未說明／Source ID：14:50
   3. 內容：建立穩定的供應鏈並加強與雲端AI公司合作，確保新技術的市場導入及應用
      負責人：未說明／截止日期：未說明／Source ID：05:30

   待釐清問題：（無——文字中沒有明確提出但尚未解決的問題）

   純資訊／無需行動：
   - 快閃記憶體需求循環特性：AI使需求持續增加，改變傳統循環模式 (00:04)
   - 研發持續大幅投入：去年約120多億，今年估計180-200億 (00:12)
   - 2008年金融海嘯衝擊：Flash缺貨時被供應商歸零不出貨，妥協吧救援 (00:26)
   （其餘略，實際輸出會列出所有段落）

   重複出現／升溫中的模式（Hypothesis，非明說）：
   公司歷史上多次瀕臨倒閉危機，且每次都靠同一管道（妥協吧）度過——文中直接點出
   「過去三次接近倒閉均由妥協吧救援」，是否代表結構性依賴單一供應管道的風險，
   文字本身未下結論。

2) 把步驟 1 的完整輸出（不只是待辦事項，決策/待釐清問題/純資訊/pattern 都一起帶著，
   保留完整脈絡，不要只截取待辦清單）連同目前的 Action Register 內容，
   一次餵給 action-register-maintainer：

   「請照 action-register-maintainer/SKILL.md 的定義，幫我提出這次的異動建議。

   【今天的日期】
   2026-07-17

   【目前 docs/ar/<你的檔名>.md 的完整表格內容】
   （貼上進行中／已完成／已取消三個區塊；如果還沒建立，寫「尚未建立，這是第一次」）

   【這次要處理的會議記錄/筆記】（已用 notes-to-action-digest 拆解過，完整結果如下）
   [貼上步驟 1 的完整輸出：決策事項／待辦事項／待釐清問題／純資訊／pattern]」

   → 得到「建議新增的列／建議更新的既有列／建議關注的逾期項目」，
   你自己確認後手動貼進 docs/ar/<檔名>.md——這個 skill 不會自己動手改檔案。
