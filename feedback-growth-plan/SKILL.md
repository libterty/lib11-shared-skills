...
- **對話草案**（標示為可調整的草案，不是劇本）
- **延伸應用**：如果需要的是不綁定單一事件的開放式成長/教練建議（例如季度成長規劃、職涯發展方向），而不是針對具體行為的回饋，改用 `shared-skills/docs/ai_coaching_prompt_template.html`——這個 skill 的範圍限定在有具體觀察事件的回饋，不涵蓋開放式教練建議
